...
void USART3_IRQHandler(void)//不知道在干什么，，可能是判断一次数据的多少，小于10ms的才是一次数据  中断服务函数   第一个标志位 第二个标志位加个receievedate
{
	u8 res=0;	      
...
//			PBout(5)=!PBout(5);
		...
		{ 
			if(USART3_RX_STA<USART3_MAX_RECV_LEN)	//还可以接收数据
			{
				...
				if(USART3_RX_STA==0) 
				{
					TIM_Cmd(TIM7,ENABLE);//使能定时器7  单纯的打开定时器7
				}
...
//pclk1:PCLK1时钟频率(Mhz)
//bound:波特率	  
void usart3_init(u32 bound)//设置串口3的各种参数，这里填频率
{  
