...
u8 camera_refresh(void)
{
	...
	if(ov_sta==2)//这时候已经存完了一帧数据
	{
		LCD_Scan_Dir(U2D_L2R);		//从上到下,从左到右 
		LCD_SetCursor(0x00,0x0000);	//设置光标位置 
		LCD_WriteRAM_Prepare();     //开始写入GRAM，从FIFO中	
	...
	for(j=0;j<76800;j++)//开始从GRAM中读数据
		{	
			GPIOB->CRL=0X88888888;//上拉输入的意思，CRL寄存器用于控制IO口的输入输出模式
			OV7670_RCK=0;//读数据时钟
